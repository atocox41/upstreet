...
const useCameraTracking = (ctx) => {
  ...
  globalThis.addEventListener('mousedown', e => {
    ...
    if (document.pointerLockElement) {
      ...
      if (!conversation) {
        if (!zTargetingManager.hasSelectedApp()) {
          ...
          zTargetingManager.click();
        }
      } else {
        storyManager.progressConversation();
      }
    }
    lastMouseZTime = now;
  });
  ...
  storyManager.addEventListener('conversationchange', e => {
    ...
    // console.log('conversationchange', conversation, storyTargetMesh);
    storyTargetMesh.visible = !conversation;
  });
  ...
  useFrame(() => {
    ...
    // raycast
    {
      ...
      storyTargetMesh.visible = !!result && !storyManager.getConversation();
    }
    storyTargetMesh.updateMatrixWorld();
...
const AdventureCursor = (ctx) => {
  useCameraTracking(ctx);

  const app = ctx.useApp();
  return app;
};
export default AdventureCursor;
