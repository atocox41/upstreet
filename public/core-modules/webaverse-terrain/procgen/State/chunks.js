...
export default class Chunks {
  constructor() {
    ...
    this.minSize = chunkMinSize;
    this.maxDepth = chunkMaxDepth;
  ...
  update() {
    ...
    const cameraChunkKey = `${Math.round(camera.position.x / this.minSize * 2 + 0.5)}${Math.round(camera.position.z / this.minSize * 2 + 0.5)}`;

    if (cameraChunkKey !== this.cameraChunkKey) {
      this.cameraChunkKey = cameraChunkKey;
      this.check();
    }
  ...
  getMainChunksCoordinates() {
    const camera = this.state.camera;
    const currentX = Math.round(camera.position.x / this.maxSize);
    const currentZ = Math.round(camera.position.z / this.maxSize);
  ...
  underSplitDistance(size, chunkX, chunkY) {
    const camera = this.state.camera;
    const distance = Math.hypot(camera.position.x - chunkX, camera.position.z - chunkY);
