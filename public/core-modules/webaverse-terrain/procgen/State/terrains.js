...
import { chunkMinSize, seed } from '../utils/constants.js';
...
export default class Terrains
{
  ...
  create(size, x, z, precision, bounding, customType) {
    // Create id
    const id = this.lastId ++;
    
    // Create terrain
    const iterations = this.getIterationsForPrecision(precision);
    const terrain = new Terrain(this, id, size, x, z, precision, bounding, customType);
  ...
  destroyTerrain(id) {
    ...
    if (terrain) {
      ...
      this.events.emit('destroy', terrain);
    }
  }
