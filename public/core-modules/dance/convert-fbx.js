...
export const convertFbx = async () => {
  ...
  // console.log('load 1', fbxDst);
  const dstAnimation = fbxDst.animations[0];

  const fbxSrc = await new Promise((accept, reject) => {
    fbxLoader.load('./edge.fbx', accept, function onprogress() {}, reject);
  });
  ...
  const boneMapping = {
    ...
    m_avg_Pelvis: 'mixamorigHips',
    m_avg_Spine1: 'mixamorigSpine',
    m_avg_Spine2: 'mixamorigSpine1',
    m_avg_Spine3: 'mixamorigSpine2',

    m_avg_Neck: 'mixamorigNeck',
    m_avg_Head: 'mixamorigHead',

    m_avg_L_Hip: 'mixamorigLeftUpLeg',
    m_avg_L_Knee: 'mixamorigLeftLeg',
    m_avg_L_Ankle: 'mixamorigLeftFoot',
    m_avg_L_Foot: 'mixamorigLeftToeBase',

    m_avg_R_Hip: 'mixamorigRightUpLeg',
    m_avg_R_Knee: 'mixamorigRightLeg',
    m_avg_R_Ankle: 'mixamorigRightFoot',
    m_avg_R_Foot: 'mixamorigRightToeBase',

    m_avg_L_Collar: 'mixamorigLeftShoulder',
    m_avg_L_Shoulder: 'mixamorigLeftArm',
    m_avg_L_Elbow: 'mixamorigLeftForeArm',
    m_avg_L_Wrist: 'mixamorigLeftHand',
    m_avg_L_Hand: null,

    m_avg_R_Collar: 'mixamorigRightShoulder',
    m_avg_R_Shoulder: 'mixamorigRightArm',
    m_avg_R_Elbow: 'mixamorigRightForeArm',
    m_avg_R_Wrist: 'mixamorigRightHand',
    m_avg_R_Hand: null,
  };
  ...
  for (let i = 0; i < srcAnimation.tracks.length; i++) {
    const track = srcAnimation.tracks[i];
    ...
    if (type === 'position') {
      ...
      const dstTrack = findDstTrack(name, type);
      if (dstTrack || dstTrack === null) {
        numMatches++;
      }
    } else {
      // skip
  ...
  if (numMatches !== Object.keys(boneMapping).length) {
    throw new Error('failed to match all tracks: ' + numMatches + ' ' + Object.keys(boneMapping).length);
  }
};
