  ...
  const wearupdate = e => {
    if (e.wear) {
      ...
      if (wearSpec) {
        ...
        const _disableAppPhysics = () => {
          ...
          // if (!app.hasComponent('pet')) {
            ...
            for (const physicsObject of physicsObjects) {
              ...
              console.log('disable actor', physicsObject);
        ...
        //   loadoutIndex,
        // } = wearSpec;
        const loadoutIndex = loadoutManager.getNextFreeIndex();
        loadoutManager.setApp(loadoutIndex, app);
        loadoutManager.setSelectedIndex(loadoutIndex);
        
        // glb wearable skinnnig
  ...
  const _unwear = () => {
    ...
    if (wearSpec) {
      ...
      const _enableAppPhysics = () => {
        // don't enable physics if the app is a pet
        // if (!app.hasComponent('pet')) {
          const physicsObjects = physicsTracker.getAppPhysicsObjects(app);
          // const physicsScene = physicsManager.getScene();
          for (const physicsObject of physicsObjects) {
      ...
      // remove from app loadout

      // const {
      //   loadoutIndex,
      // } = wearSpec;
      const loadoutIndex = loadoutManager.getIndexForApp(app);
      loadoutManager.unsetApp(loadoutIndex);
