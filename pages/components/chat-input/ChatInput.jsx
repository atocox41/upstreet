...
const triggerButtonLayers = [
  ...
  },
  {
    ...
    buttons: [
      {
        label: `Prompt`,
        prefix: `generate world prompt`,
      },
      {
        label: `File...`,
        prefix: `generate world file`,
      },
      {
        label: `Capture...`,
        prefix: `generate world capture`,
      },
    ],
  },
  {
  ...
  },
  {
    ...
    buttons: [
      ...
      {
        label: 'Follow',
        prefix: 'camera follow',
      },
      {
        label: 'Cinematic',
        prefix: 'camera cinematic',
      },
    ],
  },
];
...
export const ChatInput = ({
  ...
  const handleCommand = command => {
    ...
    switch (word0) {
      ...
      case 'connect': {
        // connect room <number>
        // connect url <path or URL>
        const word1 = words[1];
        const word2 = words[2];
        if (word1 === 'room') {
          const roomIndex = parseInt(word2, 10) - 1;
          if (roomIndex >= 0 && roomIndex < rooms.length) {
            const {src, room} = rooms[roomIndex];
            const url = `/?src=${encodeURIComponent(scenesBaseUrl + src)}&room=${room}`;
            ioBus.postMessage('connectToUrl', {
              url,
            });
            return true;
          }
      ...
      case 'livechat': {
        // const onOff = parseCommandUrl(command);
        const word1 = words[1];
        // console.log('got command 1', {command, onOff});
        if (word1 === 'on') {
          const prompt = parseCommandUrl2(command);
          // console.log('got command 2', {command, prompt});
          if (prompt) {
      ...
      case 'camera': {
        const word1 = words[1];
        if (['normal', 'free', 'follow', 'cinematic'].includes(word1)) {
          IoBus.request({
            contentWindow: globalThis.parent,
  ...
  const commitValue = value => {
    if (value.startsWith('/')) {
      ...
      if (handleCommand(command)) {
        ...
        // console.log('got trigger button', triggerButton);
        // if (triggerButton.prefix === void 0) {
        //   debugger;
        // }
        const newValue = '/' + triggerButton.prefix;
        const newCommand = newValue.slice(1);
        // console.log('got new value', {newValue, newCommand});
