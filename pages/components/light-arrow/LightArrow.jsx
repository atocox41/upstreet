...
const _downloadArrowImage = async (u) => {
...
/* let downloadArrowPromise = null;
const ensureArrowImage = async () => {
  if (!downloadArrowPromise) {
    downloadArrowPromise = _downloadArrowImage();
  }
  return await downloadArrowPromise;
}; */
let arrowsImage = null;
let downArrowsImage = null;
...
export const LightArrow = function({
  ...
  useEffect(() => {
    ...
    if (canvas) {
      ...
      let interval = null;
      (async () => {
        if (direction === 'upLeft' && !arrowsImage) {
          (async () => {
            arrowsImage = await _downloadArrowImage('/images/arrows.png');
          })();
        }
        if (direction === 'down' && !downArrowsImage) {
          (async () => {
            downArrowsImage = await _downloadArrowImage('/images/arrows2.png');
          })();
        }

        let index = 0;
        const _recurse = () => {
          const neededImage = direction === 'upLeft' ? arrowsImage : downArrowsImage;
          if (neededImage !== null) {
            const x = (index % numFramesPerRow) * frameSize;
            const y = Math.floor(index / numFramesPerRow) * frameSize;
            ctx.drawImage(neededImage, x, y, frameSize, frameSize, 0, 0, frameSize, frameSize);
