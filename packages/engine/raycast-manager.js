...
export class RaycastManager extends EventTarget {
  constructor({
    ...
    if (!engineRenderer || !cameraManager || !physicsTracker || !domRenderer) {
  ...
  update() {
    ...
    this.domRenderer.onBeforeRaycast();
    ...
    if (raycaster) {
      ...
      if (result) {
        ...
        if (pair) {
          ...
          // check dom renderer
          const object = this.domRenderer.getPhysicsObjects().find(o => o.physicsId === result.objectId);
          if (object) {
      ...
      this.domRenderer.onAfterRaycast();
    } /* else {
