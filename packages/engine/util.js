...
export function cleanLowercase(s) {
...
export function abortableRead(reader, signal) {
  return new Promise((accept, reject) => {
    const onabort = () => {
      console.log('abortable read abort');
...
// returns the ethereum network name (from the url) if it's a valid network, otherwise 'mainnet'
export const getNetwork = () => {
  ...
  // match goerli.
  const match = window.location.href.match(/^(?:https?:\/\/)?([a-z0-9\-\.]+)/i);
  if (match) {
