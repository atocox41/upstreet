...
export class AvatarCharacterSfx {
  constructor({
    ...
    const wearupdate = e => {
      this.playSoundName(e.wear ? 'itemEquip' : 'itemUnequip');
    };
  ...
  playSound(audioSpec, opts) {
    if (this.character.constructor === LocalPlayer) {
      return this.sounds.playSound(audioSpec, opts);
    } else {
      return this.sfxManager.playSpatialSound(audioSpec, this.character.characterPhysics.characterController, opts);
    }
  }
  playSoundName(soundName, opts) {
    if (this.character.constructor === LocalPlayer) {
      return this.sounds.playSoundName(soundName, opts);
    } else {
      return this.sfxManager.playSpatialSoundName(soundName, this.character.characterPhysics.characterController, opts);
    }
  }
  ...
  update(timestamp, timeDiffS) {
    ...
    const _handleSwim = () => {
      const hasSwim = this.character.actionManager.hasActionType('swim');
      const hasSprint = this.character.actionManager.hasActionType('sprint');
      if(hasSwim){
        const animationTime = avatarsWasmManager.physxWorker.getActionInterpolantAnimationAvatar(this.character.avatar.animationAvatarPtr, 'movements', 0);
        if(!hasSprint){ // breaststroke
          if(
            this.setSwimmingHand && 
            animationTime % breaststrokeDuration <= breaststrokeOffset
          ){
          ...
          if(
            this.setSwimmingHand && 
            animationTime % freestyleDuration <= freestyleOffset
          ){
            this.playSound(audioSpec);
            this.setSwimmingHand = false;
          }
          else if(!this.setSwimmingHand && 
            animationTime % freestyleDuration > freestyleOffset
          ){
            this.playSound(audioSpec);
            this.setSwimmingHand = true;
    ...
    const _handleNarutoRun = () => {
      ...
      if(this.character.avatar.narutoRunState){
        if(this.narutoRunStartTime===0){
          ...
          this.playSound(soundFiles.sonicBoom[0], {
            volume: 0.5,
          });
          this.playGrunt('narutoRun');
        }
        else {
          // if naruto run turn, play sonic boom
          if(this.arr.reduce((a,b)=>a+b) >= Math.PI/3){
            ...
            if(timeSeconds - this.narutoRunTurnSoundStartTime>soundFiles.sonicBoom[3].duration-0.9 || this.narutoRunTurnSoundStartTime===0){
              this.playSound(soundFiles.sonicBoom[3], {
                volume: 0.5,
              });
          ...
          if(timeSeconds - this.narutoRunTrailSoundStartTime>soundFiles.sonicBoom[2].duration-0.2 || this.narutoRunTrailSoundStartTime===0){
            // play sonic boom trail
            const localSound = this.playSound(soundFiles.sonicBoom[2], {
              volume: 0.5,
            });
      ...
      if(!this.character.avatar.narutoRunState && this.narutoRunStartTime!==0){
        ...
        this.narutoRunTrailSoundStartTime=0;
        this.narutoRunTurnSoundStartTime=0;
        this.playSound(soundFiles.sonicBoom[1], {
          volume: 0.5,
        });
