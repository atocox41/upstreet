...
import FakeAvatar from './avatars/fake-avatar.js';
import {
  getEyePosition,
...
export function applyCharacterModesToAvatar(character, session, rig) {
  ...
  // if (character.xrRoot.length() > 0) {
  //   console.log('bottom enabled', rig.getTopEnabled(), rig.velocity.length());
  // }
}
...
export function makeAvatar(app, environmentManager) {
  if (app.appType === 'vrm') {
    ...
    const avatar = new FakeAvatar(app);
    return avatar;
  } else {
    debugger;
    throw new Error('invalid app type for avatar: ' + app.appType);
  }
}
export function applyCharacterActionsToAvatar(character, rig) {
  ...
  rig.idleBias = (character.playerIdInt % 101) / 100;
}
export function applyCharacterChestToAvatar(character, rig) {
  if (character.chestTargetEnabled) {
    rig.chestTarget.copy(character.chestTarget);
    rig.chestTargetEnabled = true;
    return true;
  } else {
    rig.chestTargetEnabled = false;
    return false;
  }
}
...
export function applyMirrorsToAvatar(character, rig, mirrors) {
  ...
  const closestMirror = Array.from(mirrors).sort((a, b) => {
...
export function applyFacePoseToAvatar(character, rig) {
  const actionsArray = character.actionManager.getActionsArray();
  const facePoseActions = actionsArray.filter(a => a.type === 'facepose');
...
export function applyCharacterToAvatar(character, session, rig, mirrors) {
  ...
  applyCharacterChestToAvatar(character, rig);
