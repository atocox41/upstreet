...
export class QueueInterpolant extends EventTarget {
  constructor(timeDelay, bufferSize) {
    ...
    this.snapshots = _makeSnapshots(() => null, bufferSize);
    this.snapshotWriteIndex = 0;
    this.numSnapshots = 0;
  ...
  update(timestamp) {
    ...
    // console.log('effective read time', effectiveReadTime);
    ...
    for (let i = snapshotStartIndex; i !== snapshotEndIndex; i = mod(i + 1, this.bufferSize)) {
      ...
      if (effectiveReadTime >= snapshot.timestamp) {
        ...
        // console.log('num snaps down', this.numSnapshots);
        this.numSnapshots--;
      } else {
        break;
      }
  ...
  snapshot(state, timestamp) {
    // flush old snapshots to avoid queue overflow
    while (this.numSnapshots >= this.bufferSize) {
      const snapshotStartIndex = mod(this.snapshotWriteIndex - this.numSnapshots, this.bufferSize);
      const snapshot = this.snapshots[snapshotStartIndex];
      const fakeUpdateTimestamp = snapshot.timestamp + this.timeDelay;
      this.update(fakeUpdateTimestamp);
    }

    // push new snapshot
    if (this.numSnapshots < (this.bufferSize - 1)) {
      const writeSnapshot = this.snapshots[this.snapshotWriteIndex];
