...
// import {lightingManager} from '../managers/lights/lights-manager.js';
...
class DioramaRenderer {
  ...
  render(timeOffset, timeDiff, width, height, objects, target, cameraOffset, canvases, opts) {
    ...
    const _addRootLightsToScene = scene => {
      const restoreFn = [];
      for (const light of this.lightingManager.lights) {
    ...
    const _pushState = () => {
      ...
      const _restoreParents = () => {
        ...
        if (lights) {
          for (const autoLight of this.autoLights) {
            autoLight.parent && autoLight.parent.remove(autoLight);
          }
        }
      };
...
class PlayerDiorama {
  constructor(opts) {
    ...
    this.width = 0;
    this.height = 0;
    this.camera = dioramaRenderer.sideCamera;
    this.dioramaRenderer = dioramaRenderer;
    // this.loaded = false;
  ...
  updateAspect() {
    ...
    if (this.dioramaRenderer.sideCamera.aspect !== newAspect) {
      this.dioramaRenderer.sideCamera.aspect = newAspect;
      this.dioramaRenderer.sideCamera.updateProjectionMatrix();
    }
  }
  removeCanvas(canvas) {
  ...
  setCameraOffset(newCameraOffset) {
    this.cameraOffset.copy(newCameraOffset);
  }
  ...
  update(timestamp, timeDiff) {
    ...
    if (this.canvases.length === 0) {
      this.lastDisabledTime = timestamp;
      return;
    }
    const timeOffset = timestamp - this.lastDisabledTime;

    // console.log('render', {
    //   timeOffset, timeDiff, width: this.width, height: this.height, objects, target, cameraOffset, canvases,
    // });
    this.dioramaRenderer.render(
      timeOffset,
      timeDiff,
      this.width,
      this.height,
...
export const createPlayerDiorama = (opts) => {
  const diorama = new PlayerDiorama(opts);
  // if (!detached) {
