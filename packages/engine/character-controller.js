...
import {
  ElevenLabsVoiceEndpoint,
  TiktalknetVoiceEndpoint,
  VoiceEndpoint,
  VoiceEndpointVoicer,
...
class Character extends THREE.Object3D {
  constructor({
    ...
    // if (!engine || !engineRenderer || !audioManager || !hupsManager || !sounds || !voices || !physicsTracker || !environmentManager /*|| !xrManager*/) {
    ...
    //   debugger;
    // }
    if (!importManager || !appContextFactory || !voices || !sfxManager) {
      debugger;
    }
    ...
    this.appManager.name = 'character';
  ...
  setVoiceEndpoint(model, voiceId) {
    if (typeof model !== 'string' || typeof voiceId !== 'string') {
      throw new Error('voice Id is null');
    }

    switch (model) {
      case 'elevenlabs': {
        this.voiceEndpoint = new ElevenLabsVoiceEndpoint({
          voiceId,
          voices: this.voices,
        });
        break;
      ...
      case 'tiktalknet': {
        this.voiceEndpoint = new TiktalknetVoiceEndpoint({
          voiceId,
          voices: this.voices,
        });
        break;
      ...
      default: {
        debugger;
        throw new Error('invalid voice model: ' + model);
      }
    }
  ...
  updateVoicer() {
    const voice = this.voiceEndpoint || this.voicePack;
  ...
  setImageEndpoint(imageEndpoint) {
  ...
  wear(app, {
    ...
    // console.log('wear app', app, loadoutIndex);
    // debugger;
    ...
    if (loadoutIndex >= 0 && loadoutIndex < numLoadoutSlots) {
      ...
      const _transplantAppFromWorldToPlayer = () => {
        const worldAppManager = app.parent;
        const playerAppManager = this.appManager;
        worldAppManager.transplantApp(app, playerAppManager);
      ...
      this.applyWear(app, {loadoutIndex});
    }
  }

  unapplyWear(app, {loadoutIndex, dropStartPosition, dropDirection, transplantFn}) {
    // console.log('unapply wear', app, loadoutIndex, dropStartPosition, dropDirection, transplantFn);
    ...
    // Transplant the app if have callback.
    if (transplantFn) {
      transplantFn();
    }
  ...
  unwear(app, {
    ...
    // console.log('unwear app', app, destroy, dropStartPosition, dropDirection);
    if (!dropStartPosition) {
      debugger;
    }
    ...
    if (wearAction) {
      ...
      this.actionManager.removeAction(wearAction);

      const _transplantAppFromPlayerToWorld = () => {
        const playerAppManager = app.parent;
        const worldAppManager = this.engine.appManagerContext.getAppManager();
        playerAppManager.transplantApp(app, worldAppManager);
      ...
      this.unapplyWear(app, {
        loadoutIndex,
        dropStartPosition,
        dropDirection,
        transplantFn: _transplantAppFromPlayerToWorld,
      });
    }
  }
  ...
  setTarget(target) { // set both head and eyeball target;
    if (target) {
      ...
      this.chestTarget.copy(target);
      this.chestTargetEnabled = true;
    } else {
      this.headTargetEnabled = false;
      this.eyeballTargetEnabled = false;
      this.chestTargetEnabled = false;
...
class StateCharacter extends Character {
  constructor(opts) {
    ...
    this.appManager.name = this.appManager.name + ' ' + playerId;
  }
...
class AvatarCharacter extends StateCharacter {
  constructor(opts) {
    ...
    this.voiceInput.addEventListener('speech', e => {
      const {data} = e;
      const {transcript} = data;
      const o = {
        playerName: this.playerSpec.name,
        message: transcript,
      };
      const m = createAnonymousChatMessage(o);
      this.engine.chatManager.addMessage(m);
    });
  ...
  async setPlayerSpec(playerSpec) {
    if (playerSpec) {
      const lastPlayerSpec = this.playerSpec;
      this.lastPlayerSpec = lastPlayerSpec;
      this.playerSpec = playerSpec;

      const promises = [];
      if (playerSpec.avatarUrl !== lastPlayerSpec.avatarUrl) {
        const loadAvatarPromise = (async () => {
          await this.loadAvatar(playerSpec.avatarUrl);

          const _setCharacterPhysicsObjectLore = () => {
            this.characterPhysics.characterController.name = playerSpec.name;
            this.characterPhysics.characterController.description = playerSpec.bio;
          };
          _setCharacterPhysicsObjectLore();
        })();
        promises.push(loadAvatarPromise);
      }
      
      const _loadVoicePack = async () => {
        const voicePackName = playerSpec.voicePack;
        if (voicePackName !== lastPlayerSpec.voicePack) {
          if (voicePackName) {
            const defaultVoicePackName = 'ShiShi voice pack';
            const voicePack = this.voices.voicePacks.find(vp => vp.name === voicePackName) ??
              this.voicePacks.find(vp => vp.name === defaultVoicePackName);

            if (!voicePack) {
              console.warn('no such voice pack', {voicePackName, defaultVoicePackName});
              debugger;
            }
      ...
      const loadVoicePackPromise = _loadVoicePack();
      promises.push(loadVoicePackPromise);

      const _loadVoiceEndpoint = () => {
        let voiceEndpointString = playerSpec.voiceEndpoint;
        if (!voiceEndpointString) {
          const defaultVoiceModel = Object.keys(this.voices.voiceEndpoints)[0];
          const defaultVoiceName = this.voices.voiceEndpoints[defaultVoiceModel][0].name;
          if (!defaultVoiceModel || !defaultVoiceName) {
            debugger;
            throw new Error('no default voice model or name');
          }
          const defaultVoiceEndpoint = `${defaultVoiceModel}:${defaultVoiceName}`;
          voiceEndpointString = defaultVoiceEndpoint;
        }

        if (voiceEndpointString !== lastPlayerSpec.voiceEndpoint) {
          const match = (voiceEndpointString || '').match(/^(.+?):(.+?)$/);
          if (match) {
            const model = match[1];
            const voiceName = match[2];

            const voiceEndpoints = this.voices.voiceEndpoints[model];
            if (voiceEndpoints) {
              const voiceEndpoint = voiceEndpoints.find(ve => ve.name === voiceName);
              if (voiceEndpoint) {
                const {
                  voiceId,
                } = voiceEndpoint;
                // console.log('set voice endpoint', {model, voiceId});
                this.setVoiceEndpoint(model, voiceId);
              } else {
                console.warn('no such voice name', {model, voiceName});
              }
            } else {
              console.warn('no such voice model', {model});
            }
          } else {
            debugger;
            console.warn('could not parse voice', {voiceEndpointString});
          }
        }
      };
      _loadVoiceEndpoint();
      ...
      this.dispatchEvent({
        type: 'playerspecupdate',
        playerSpec,
      });
    // } else {
    //   throw new Error('player spec is required')
    }
  }
  ...
  async waitForAvatar() {
    if (!this.avatar) {
      await new Promise((accept, reject) => {
        ...
        this.addEventListener('avatarchange', avatarchange);

        const cleanup = () => {
          this.removeEventListener('avatarchange', avatarchange);
        };
      });
    }
  }
  ...
  updateAvatar(timestamp, timeDiff, session, xrAvatarPose) {
    if (this.avatar) {
      const timeDiffS = timeDiff / 1000;

      this.characterBehavior.update(timestamp, timeDiff);
      this.characterPhysics.update(timestamp, timeDiffS, xrAvatarPose);

      if (session && xrAvatarPose) {
        this.avatar.setLocalAvatarPose(this, xrAvatarPose);
      }
      ...
      this.avatar.update(timestamp, timeDiff, session);
  ...
  constructor(opts) {
    ...
    this.actionInterpolant.addEventListener('statechange', e => {
      ...
      if (action) {
        ...
        if (this.actionManager.hasActionId(actionId)) {
...
class LocalPlayer extends UninterpolatedPlayer {
  ...
  async playAudioStream(stream, opts) {
    const {audioContext} = this.audioManager;
    this.avatar.setAudioStreamEnabled({
      audioContext,
    });
    const audioInjectWorkletNode = this.avatar.getAudioStreamInput();
    ...
    await this.voicer.start(stream, {
