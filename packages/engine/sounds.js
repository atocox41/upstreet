...
const soundFiles = {
  ...
  companionEmote: _getSoundFiles(/^companionEmote\//),
};
...
export class Sounds {
  ...
  constructor({
    ...
    /* this.ioBus && this.ioBus.registerHandler('playSoundName', e => {
      const {name} = e;
      this.playSoundName(name);
    }); */
  ...
  playSound(audioSpec, {
    ...
    if (volume !== 1) {
      ...
      gainNode.connect(audioContext.gain);
    } else {
