...
// import {
//   AppContext,
// } from '../app-runtime/app-context.js';
...
class AppManager extends THREE.Object3D {
  constructor({
    ...
    // members
    if (!importManager || !appContextFactory) {
      console.warn('import manager missing', importManager, new Error().stack);
      debugger;
    }
  ...
  /* async loadScnFromJson(j) {
    ...
    for (let i = 0; i < objects.length; i++) {
      const p = (async () => {
        ...
        const contentId = this.importManager.getObjectUrl({
          contentId: start_url,
          type,
          content,
        }, baseUrl);
  ...
  save(opts) {
    ...
    // XXX actually filter the apps here

    const objects = apps.map(app => app.spec);
    return {
      objects,
    };
  }
  async load(j) {
    ...
    for (const object of objects) {
      ...
      if (spec.position) {
        spec.position = new THREE.Vector3().fromArray(spec.position);
      }
      if (spec.quaternion) {
        spec.quaternion = new THREE.Quaternion().fromArray(spec.quaternion);
      }
      if (spec.scale) {
        spec.scale = new THREE.Vector3().fromArray(spec.scale);
      }
      const app = await this.addAppAsync(spec);
    }
  }
  ...
  async addAppAsync({
    ...
    appContext = this.appContextFactory.makeAppContext({
      app,
    }),
    ...
    if (contentId) {
      console.warn('contentId is deprecated');
      debugger;
    }
    ...
    app = await this.importManager.createAppAsync({
      start_url,
      type,
      content,
      position,
