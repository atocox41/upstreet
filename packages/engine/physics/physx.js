...
const physxWorker = (() => {
  ...
  w.destroyScene = (sceneAddress) => {
    Module._destroyPhysics(sceneAddress);
  }
  ...
  w.raycastPhysics = (physics, p, q) => {
    if (physics) {
      ...
      const isHit = scratchStack.u32[6];
      let collision = null;
      if (isHit) {
        collision = {
  ...
  w.sweepBoxesArray = (
    ...
    const _origins = new Float32Array(
      scratchStack.f32.buffer,
      scratchStack.f32.byteOffset,
      n * 3
    );
    const _quaternions = new Float32Array(
      scratchStack.f32.buffer,
      scratchStack.f32.byteOffset + n * 3 * Float32Array.BYTES_PER_ELEMENT,
      n * 4
    );
    const _halfExtents = new Float32Array(
      scratchStack.f32.buffer,
      scratchStack.f32.byteOffset + n * (3 + 4) * Float32Array.BYTES_PER_ELEMENT,
      n * 3
    );
    const _directions = new Float32Array(
      scratchStack.f32.buffer,
      scratchStack.f32.byteOffset + n * (3 + 4 + 3) * Float32Array.BYTES_PER_ELEMENT,
      n * 3
    );
    ...
    for (let i = 0; i < n; i++) {
      // Position
      origins[i].toArray(_origins, i * 3);
      quaternions[i].toArray(_quaternions, i * 4);
      halfExtents[i].toArray(_halfExtents, i * 3);
      directions[i].toArray(_directions, i * 3);
    }

    // Output
    const originsOffset = scratchStack.f32.byteOffset;
    const quaternionsOffset = scratchStack.f32.byteOffset + 3 * Float32Array.BYTES_PER_ELEMENT * n;
    const halfExtentsOffset = scratchStack.f32.byteOffset + (3 + 4) * Float32Array.BYTES_PER_ELEMENT * n;
    const directionsOffset = scratchStack.f32.byteOffset + (3 + 4 + 3) * Float32Array.BYTES_PER_ELEMENT * n;
    
    const hitsOffset = scratchStack.u32.byteOffset + (3 + 4 + 3 + 3) * Float32Array.BYTES_PER_ELEMENT * n;
    const positionOffset = scratchStack.f32.byteOffset + (3 + 4 + 3 + 3 + 1) * Float32Array.BYTES_PER_ELEMENT * n;
    const normalOffset = scratchStack.f32.byteOffset + (3 + 4 + 3 + 3 + 1 + 3) * Float32Array.BYTES_PER_ELEMENT * n;
    const distanceOffset = scratchStack.f32.byteOffset + (3 + 4 + 3 + 3 + 1 + 3 + 3) * Float32Array.BYTES_PER_ELEMENT * n;
    const objectIdOffset = scratchStack.u32.byteOffset + (3 + 4 + 3 + 3 + 1 + 3 + 3 + 1) * Float32Array.BYTES_PER_ELEMENT * n;
    const faceIndexOffset = scratchStack.u32.byteOffset + (3 + 4 + 3 + 3 + 1 + 3 + 3 + 1 + 1) * Float32Array.BYTES_PER_ELEMENT * n;

    // +EMSCRIPTEN_KEEPALIVE void sweepBoxesArray(
    ...
    // console.log('got slice', scratchStack.u32.slice((3 + 4 + 3 + 3) * n, (3 + 4 + 3 + 3) * n + n));

    return {
      hit: scratchStack.u32.slice((3 + 4 + 3 + 3) * n, (3 + 4 + 3 + 3) * n + n),
      position: scratchStack.f32.slice((3 + 4 + 3 + 3 + 1) * n, (3 + 4 + 3 + 3 + 1) * n + n * 3),
      normal: scratchStack.f32.slice((3 + 4 + 3 + 3 + 1 + 3) * n, (3 + 4 + 3 + 3 + 1 + 3) * n + n * 3),
      distance: scratchStack.f32.slice((3 + 4 + 3 + 3 + 1 + 3 + 3) * n, (3 + 4 + 3 + 3 + 1 + 3 + 3) * n + n),
      objectId: scratchStack.u32.slice((3 + 4 + 3 + 3 + 1 + 3 + 3 + 1) * n, (3 + 4 + 3 + 3 + 1 + 3 + 3 + 1) * n + n),
      faceIndex: scratchStack.u32.slice((3 + 4 + 3 + 3 + 1 + 3 + 3 + 1 + 1) * n, (3 + 4 + 3 + 3 + 1 + 3 + 3 + 1 + 1) * n + n),
    };
  };
