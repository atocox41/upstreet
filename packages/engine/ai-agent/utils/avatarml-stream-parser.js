...
export class Parser /*extends EventTarget*/ {
  constructor() {
    this.text = '';
  }

  #parseMatch(match) {
    const [_, characterName, commandString, message] = match;
    const commandMatch = commandString.match(/^(.+?)(?:=(.+?))?$/);
    let command, commandArgument;
    if (commandMatch) {
      command = commandMatch[1];
      commandArgument = commandMatch[2];
    } else {
  ...
  write(s) {
    ...
    let match;
    while (match = this.text.match(/^:(.+)::(.+):::(.*)\n/)) {
      const e = this.#parseMatch(match);
      events.push(e);
