...
import {emoteAnimations} from '../animationHelpers.js';
...
export const numFrames = 7;
export const numAngles = 8;
const worldSize = 2;
...
export const getFrameOffsetCoords = (angleIndex) => {
  let x = angleIndex % numSlots;
  let y = (angleIndex - x) / numSlots;
  x *= texSize;
  y *= texSize;
  return [x, y];
};
...
class SpriteAnimationPlaneMesh extends THREE.Mesh {
  constructor(tex, {angleIndex}) {
    const planeSpriteMaterial = new THREE.ShaderMaterial({
      uniforms: {
        uTex: {
          // type: 't',
          value: tex,
...
export const renderSpriteImages = async (arrayBuffer, srcUrl) => {
  ...
  const localRig = new Avatar(avatarQuality, {
    fingers: true,
    hair: true,
    visemes: true,
    debug: false,
  ...
  const model = avatarQuality.scene;
