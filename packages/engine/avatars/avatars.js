...
class Avatar extends AvatarBase {
    ...
    this.chestTarget = new THREE.Vector3();
    this.chestTargetPlane = new THREE.Plane();
    this.chestTargetEnabled = false;

    this.springBoneManager = object.userData.vrm?.springBoneManager;
    if (this.springBoneManager){
      this.springBoneManager.setInitState();
    }

    const _getOffset = (bone, parent = bone?.parent) =>
      bone && bone.getWorldPosition(new THREE.Vector3())
    ...
    if (this.options.visemes) {
      ...
      // vrm1 format
      const _getBlendShapeBindsForPresetName = presetName => {
        ...
        if (preset && preset.morphTargetBinds && preset.morphTargetBinds.length > 0) {
          return preset.morphTargetBinds;
        } else {
          return null;
        }
      };
      ...
      // vrm0 format
      const _getBlendShapeBindsForName = (name, skinnedMesh) => {
        ...
        if (Array.isArray(blendShapes)) {
          const shape = blendShapes.find(blendShape => blendShape.name.toLowerCase() === name.toLowerCase());
          if (shape && shape.binds && shape.binds.length > 0) {
            const binds = shape.binds.filter(bind => {
              const nodesUsingMesh = [];
              this.object.parser.json.nodes?.forEach((node, i) => {
              ...
              let found = false;
              nodesUsingMesh.some(nodeIndex => {
                const nodes = this.object.parser.cacheSync.get('node');
      ...
      this.skinnedMeshesVisemeMappings = this.skinnedMeshes.map(skinnedMesh => {
        const {morphTargetDictionary, morphTargetInfluences} = skinnedMesh;
        if (morphTargetDictionary && morphTargetInfluences) {
          let blinkLeftBinds, blinkRightBinds, aBinds, eBinds, iBinds, oBinds, uBinds, neutralBinds, angryBinds, funBinds, joyBinds, sorrowBinds, surpriseBinds;
          if (isVrmVersion0) {
            blinkLeftBinds = _getBlendShapeBindsForName('blink_l', skinnedMesh);
            blinkRightBinds = _getBlendShapeBindsForName('blink_r', skinnedMesh);
            aBinds = _getBlendShapeBindsForName('a', skinnedMesh);
            eBinds = _getBlendShapeBindsForName('e', skinnedMesh);
            iBinds = _getBlendShapeBindsForName('i', skinnedMesh);
            oBinds = _getBlendShapeBindsForName('o', skinnedMesh);
            uBinds = _getBlendShapeBindsForName('u', skinnedMesh);
            neutralBinds = _getBlendShapeBindsForName('neutral', skinnedMesh);
            angryBinds = _getBlendShapeBindsForName('angry', skinnedMesh);
            funBinds = _getBlendShapeBindsForName('fun', skinnedMesh);
            joyBinds = _getBlendShapeBindsForName('joy', skinnedMesh);
            sorrowBinds = _getBlendShapeBindsForName('sorrow', skinnedMesh);
            surpriseBinds = _getBlendShapeBindsForName('suprise', skinnedMesh) || _getBlendShapeBindsForName('surprise', skinnedMesh) || _getBlendShapeBindsForName('surprised', skinnedMesh);
          } else {
            blinkLeftBinds = _getBlendShapeBindsForPresetName('blink_l');
            blinkRightBinds = _getBlendShapeBindsForPresetName('blink_r');
            aBinds = _getBlendShapeBindsForPresetName('a');
            eBinds = _getBlendShapeBindsForPresetName('e');
            iBinds = _getBlendShapeBindsForPresetName('i');
            oBinds = _getBlendShapeBindsForPresetName('o');
            uBinds = _getBlendShapeBindsForPresetName('u');
            neutralBinds = _getBlendShapeBindsForPresetName('neutral');
            angryBinds = _getBlendShapeBindsForPresetName('angry');
            funBinds = _getBlendShapeBindsForPresetName('fun');
            joyBinds = _getBlendShapeBindsForPresetName('joy');
            sorrowBinds = _getBlendShapeBindsForPresetName('sorrow');
            surpriseBinds = _getBlendShapeBindsForPresetName('surprised');
          }
          // const extraBinds = _getBlendShapeIndexForName('extra');
          return [
  ...
  static bindAvatar(object) {
    ...
    const root = object.userData.vrm?.humanoid?.humanBones?.hips?.node;
    fixSkeletonZForward(root, {
      preRotations,
    });
    model.traverse(o => {
      if (o.isSkinnedMesh) {
  ...
  initializeBonePositions(setups) {
    ...
    if (setups.chest) this.shoulderTransforms.chest.position.copy(setups.chest);
    if (setups.upperChest) this.shoulderTransforms.upperChest.position.copy(setups.upperChest);
    if (setups.neck) this.shoulderTransforms.neck.position.copy(setups.neck);
  ...
  setLocalAvatarPose(character, poseArray) {
    const [
    ...
    // const positionOffset = character.position.clone();
    // if (character.avatar) {
    //   positionOffset.y -= character.avatar.height;
    // }

    const positionOffset = character.position.clone()
      .sub(character.xrRoot)

    this.inputs.hmd.position.fromArray(hmdPosition)
      .add(positionOffset);
    this.inputs.hmd.quaternion.fromArray(hmdQuaternion)
      // .multiply(character.quaternion);
    this.poseManager.vrTransforms.head.position.copy(this.inputs.hmd.position);
    this.poseManager.vrTransforms.head.quaternion.copy(this.inputs.hmd.quaternion);

    // console.log('got hmd', this.inputs.hmd.position.toArray());
  ...
  update(timestamp, timeDiff, session) {
    ...
    const _updateHeadTarget = () => {
      const eyePosition = getEyePosition(this.modelBones);
      const targetQuaternion = localQuaternion.setFromRotationMatrix(
      ...
      // get the fully resolved matrix of the THREE.Object3D by walking up the tree
      const getWorldMatrix = (object, target, tmpMatrix) => {
        target.identity();
        do {
          tmpMatrix.compose(object.position, object.quaternion, object.scale);
          target.premultiply(tmpMatrix);
        } while (object = object.parent);
        return target;
      };

      const currentQuaternion = localQuaternion2;
      this.modelBoneOutputs.Neck.matrixWorld.decompose(localVector, currentQuaternion, localVector2);

      const animationQuaternion = localQuaternion3;
      const neckMatrixWorld = getWorldMatrix(this.modelBoneOutputs.Neck, localMatrix, localMatrix2);
      neckMatrixWorld.decompose(localVector3, animationQuaternion, localVector4);
      
      const resultQuaternion = localQuaternion4;

      const needsHeadTarget = this.headTargetEnabled &&
        this.modelBones.Root.quaternion.angleTo(targetQuaternion) < lookAnimationAngleLimit;
      if (needsHeadTarget && !this.lastNeedsHeadTarget) {
        this.startHeadTargetQuaternion.copy(currentQuaternion);
      ...
      if (needsHeadTarget) {
        resultQuaternion.copy(this.startHeadTargetQuaternion)
          .slerp(targetQuaternion, cubicBezier(headTargetFactor));
      } else {
        resultQuaternion.copy(this.startHeadTargetQuaternion)
          .slerp(
            // this.headTargetInverted ?
            //   localQuaternion4.setFromAxisAngle(localVector.set(0, 1, 0), Math.PI)
            // :
            //   localQuaternion4.identity(),
            animationQuaternion,
            cubicBezier(headTargetFactor)
          );

        // resultQuaternion.copy(neutralQuaternion);
      }
      this.modelBoneOutputs.Neck.matrixWorld.compose(localVector, resultQuaternion, localVector2)
    ...
    const _updateEyeballTarget = () => {
      const eyePosition = getEyePosition(this.modelBones);
      const targetQuaternion = localQuaternion2.setFromRotationMatrix(
        localMatrix.lookAt(
          this.eyeballTarget,
          eyePosition,
          upVector,
      ...
      const needEyeballTarget = this.eyeballTargetEnabled &&
        this.modelBones.Root.quaternion.angleTo(targetQuaternion) < lookAnimationAngleLimit;
    ...
    const _triggerShapes = (morphTargetInfluences, binds, weight) => {
      if (binds !== null){
        for (let i = 0; i < binds.length; i++) {
          const bind = binds[i];
          // missing to consider target mesh (done: "bind.mesh in vrm0", todo: "bind.node in vrm1")
          // morphTargetInfluences[bind.index] = weight * bind.weight; // Math.min((morphTargetInfluences[bind.index] + weight * bind.weight),1);
          morphTargetInfluences[bind.index] = weight / 100 * bind.weight;
        }
      }
    }
      ...
      for (const visemeMapping of this.skinnedMeshesVisemeMappings) {
        if (visemeMapping) {
          ...
          if (volumeValue > 0) {
            ...
            // console.log('volume value', volumeValue, this.vowels.slice());
            _triggerShapes(morphTargetInfluences,aBinds,volumeValue)
            _triggerShapes(morphTargetInfluences,eBinds,volumeValue * this.vowels[1]);
            _triggerShapes(morphTargetInfluences,iBinds,volumeValue * this.vowels[2]);
            _triggerShapes(morphTargetInfluences,oBinds,volumeValue * this.vowels[3]);
            _triggerShapes(morphTargetInfluences,uBinds,volumeValue * this.vowels[4]);
          }
          /* } else { // fake speech
          ...
          if (this.faceposes.length > 0) {
            const facepose = this.faceposes[this.faceposes.length - 1];
            const value = facepose.value ?? 1;
            const weight = (() => {
              if (typeof facepose.startTime === 'number') {
                const timeDiff = now - facepose.startTime;
                const fadeIn = (facepose.fadeIn ?? 0) * 1000;
                const duration = (facepose.duration ?? Infinity) * 1000;
                const fadeOut = (facepose.fadeOut ?? 0) * 1000;

                if (timeDiff < fadeIn) {
                  return timeDiff / fadeIn;
                } else if (timeDiff < fadeIn + duration) {
                  return 1;
                } else if (timeDiff < fadeIn + duration + fadeOut) {
                  return 1 - (timeDiff - (fadeIn + duration)) / fadeOut;
                } else {
                  return 0;
                }
              } else {
            ...
            const weightMultiplier = value * weight;
            switch (facepose.emotion) {
              case 'neutral': {
                _triggerShapes(morphTargetInfluences,neutralBinds,weightMultiplier)
                break;
              }
              case 'alert': {
                _triggerShapes(morphTargetInfluences,joyBinds,weightMultiplier)
                break;
              }
              case 'angry': {
                _triggerShapes(morphTargetInfluences,angryBinds,weightMultiplier)
                break;
              }
              case 'fun': {
                _triggerShapes(morphTargetInfluences,funBinds,weightMultiplier)
                break;
              }
              case 'joy': {
                _triggerShapes(morphTargetInfluences,joyBinds,weightMultiplier)
              ...
              case 'sorrow': {
                _triggerShapes(morphTargetInfluences,sorrowBinds,weightMultiplier)
                break;
              }
              case 'embarrassed': {
                // fun index + sorrow index
                _triggerShapes(morphTargetInfluences,funBinds,0.4 * weightMultiplier)
                _triggerShapes(morphTargetInfluences,sorrowBinds,0.4 * weightMultiplier)
                break;
              }
              case 'headNod': {
                _triggerShapes(morphTargetInfluences,funBinds,weightMultiplier)
              ...
              case 'headShake': {
                _triggerShapes(morphTargetInfluences,angryBinds,0.4 * weightMultiplier)
                _triggerShapes(morphTargetInfluences,sorrowBinds,0.4 * weightMultiplier)
                break;
              }
              case 'sad': {
              ...
              case 'surprise': {
                _triggerShapes(morphTargetInfluences,surpriseBinds,weightMultiplier)
                break;
              }
              case 'victory': {
                _triggerShapes(morphTargetInfluences,joyBinds,weightMultiplier)
              ...
              default: {
                if (facepose.emotion) {
                  ...
                  if (index !== -1) {
                    ...
                    // console.log('yes morph taget', index);
                  } /* else {
                    console.log('no morph target', index);
                  } */
                } else {
                  console.warn('invalid facepose emotion', facepose);
                }
                break;
              }
            }
          ...
          // blink
          _triggerShapes(morphTargetInfluences,blinkLeftBinds,blinkValue)
          _triggerShapes(morphTargetInfluences,blinkRightBinds,blinkValue)
        }
      }
    };
