...
export class GameManager extends EventTarget {
  ...
  constructor({
    ...
    // members
    if (!engineRenderer || !ioManager || !cameraManager || !playersManager || !loadoutManager || !raycastManager || !interactionManager || !appManagerContext || !hitManager || !dropManager || !sounds || !zTargetingManager) {
  ...
  registerHighlightMeshes() {
    ...
    this.engineRenderer.sceneLowPriority.add(this.mouseDomEquipmentHoverPhysicsMesh);
  }
  ...
  endUse() {
    ...
    if (useAction) {
      ...
      if (app) { // app might already be destroyed
        app.dispatchEvent({
          type: 'use',
          use: false,
        });
  ...
  menuBDown() {
    ...
    if (!sssAction) {
      ...
      this.sounds.playSoundName('limitBreak');
  ...
  menuSwitchCharacter() {
    ...
    if (switched) {
      this.sounds.playSoundName('menuReady');
    }
  }
  ...
  async dropTest() {
    ...
    const contentId = '/core-modules/silsword/';
    const localPlayer = this.playersManager.getLocalPlayer();
  ...
  toggleFly() {
    if(IS_FLYING_ENABLED) {
      ...
      if (localPlayer.actionManager.hasActionType('fly')) {
        ...
        localPlayer.actionManager.hasActionType('skydive') &&
  ...
  async handleDropJsonForDrop(object, currentAddress, WebaversecontractAddress, afterDrop = f => f) { // currentAddress = walletaddress, WebaversecontractAddress= signaddress
    ...
    const velocity = localVector.set(0, 0, -1).applyQuaternion(localPlayer.quaternion)
      ...
      .multiplyScalar(2.5);
    const appManager = this.appManagerContext.getAppManager();
    appManager.importAddedUserVoucherApp(position, quaternion, newObject, velocity);
  }
  ...
  menuActivateUp() {
    if (this.interactionManager.canInteractImmediate()) {
  ...
  getPlayerDiorama(outline, background) {
    if (!this.#playerDiorama) {
      this.#playerDiorama = createPlayerDiorama({
        outline: !!outline,
  ...
  saveScene() {
    const appManager = this.appManagerContext.getAppManager();
    const scene = appManager.exportJSON();
