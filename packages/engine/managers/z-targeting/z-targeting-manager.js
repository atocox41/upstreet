...
export class ZTargetingManager extends THREE.Object3D {
  ...
  isFocused() {
    return !!this.focusTargetReticle;
  }
  getFocusedApp() {
    return this.focusTargetReticle ?
  ...
  #focusSelectedApp() {
    ...
    const remoteApp = this.getFocusedApp();
    const localPlayer = this.playersManager.getLocalPlayer();
  ...
  handleDown(object = this.engineRenderer.camera) {
    if (!this.cameraManager.focus) {
  ...
  click() {
    const physicsId = this.focusTargetReticle.physicsId;
