...
export class NetworkRealms extends EventTarget {
  ...
  constructor({
    ...
    if (!endpointUrl || !playerId || !audioContext) {
      console.warn('got bad network realms args', {playerId, audioContext});
  ...
  disableMic() {
    console.log('disable mic 1', this.microphoneSource, new Error().stack);
    if (this.microphoneSource) {
      const headRealm = this.localPlayer.headTracker.getHeadRealm();
      const {networkedAudioClient} = headRealm;
  ...
  async updateRealmsKeys({
    ...
    if (!arrayEquals(realmsKeys, this.lastRealmsKeys)) {
      ...
      await this.migrateTx(async () => {
        ...
        await this.localPlayer.headTracker.tryMigrate(rootRealmKey);
