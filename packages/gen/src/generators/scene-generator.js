...
const getPlanesRgbd = async (width, height, focalLength, depthFloats32Array, minSupport = 30000) => {
  ...
  const u = new URL(`https://ai-proxy.webaverse.com/api/depth/planeDetection?minSupport=${minSupport}`);
...
const _getImageSegments = async (
  ...
  const u = new URL(`https://ai-proxy.webaverse.com/api/mask2former/predict`);
...
export async function getDepth(imageArrayBuffer) {
  ...
  const u = `https://ai-proxy.webaverse.com/api/depth/predictDepth`;
  const res = await fetch(u, {
    method: 'POST',
    body: blob,
    headers: {
...
export async function compileVirtualScene({
  ...
  if (sphericalHarmonic) {
    ...
    {
      const u = new URL(`https://ai-proxy.webaverse.com/api/irn/lighting`);
      const res = await fetch(u, {
        method: 'POST',
        body: formData,
      });
